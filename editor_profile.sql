...
SELECT compliance_key,
       ...
       C.country_name_clean AS country,
...
LEFT JOIN `unity-ai-aggregate-data-test.adhoc_public.country_mapping` C ON C.country_code=(CASE WHEN D.countryOfResidence LIKE '%=%' THEN B.C_Country ELSE D.countryOfResidence END)
