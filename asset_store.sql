...
SELECT body.ts AS timestamp,A.submit_date,A.compliance_key,body.amount_final_usd,body.purchase_type,
       body.quantity,body.sale_type,body.source,C.region AS region,
       ...
       B.* EXCEPT(compliance_key),C.country
...
LEFT JOIN `unity-ai-aggregate-data-prd.analytics.region_mapping_iso` C ON A.enrichments.geo_ip.country_code=C.iso3166
