...
FROM `unity-other-learn-prd.reynafeng.student_mau`
GROUP BY 1,2
ORDER BY 1) AS A
)
...
SELECT A.visit_month ,A.MAU ,
       ...
       A.rolling_average AS monthly_rolling_students,
       
       A.rolling_sum AS monthly_rolling_students_sum,
       IF(NOT num_start IS NULL, num_start, 0) AS student_license_start,
       IF(NOT num_end IS NULL, num_end, 0) AS student_license_end,
       IF(NOT running_balance IS NULL, running_balance,0) AS student_license_balance,
