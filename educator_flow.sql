...
hub_mg AS (
    ...
    JOIN `unity-other-learn-prd.reynafeng.educator_activation` student ON COALESCE(mapping.compliance_key,user.compliance_key)=student.compliance_key AND mapping.submit_date BETWEEN DATE(licnese_grant_time) AND DATE(licnese_expiration_time)
...
editor AS (
    SELECT COALESCE(mapping.compliance_key,user.compliance_key) AS compliance_key,
           MIN(mapping.first_login_date) AS first_editor_login
    ...
    JOIN `unity-other-learn-prd.reynafeng.educator_activation` student ON COALESCE(mapping.compliance_key,user.compliance_key)=student.compliance_key AND mapping.first_login_date BETWEEN DATE(licnese_grant_time) AND DATE(licnese_expiration_time)
    GROUP BY 1
),
