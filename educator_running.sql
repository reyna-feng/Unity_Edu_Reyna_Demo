--Update Time: 3/10 9:30 PM--
CREATE OR REPLACE TABLE `unity-other-learn-prd.reynafeng.educator_running` AS
...
user AS (
  ...
  JOIN `unity-other-learn-prd.reynafeng.educator_activation` student ON A.compliance_key = student.compliance_key
  GROUP BY 1,2,3,4
)
...
FROM(
    ...
    JOIN `unity-other-learn-prd.reynafeng.educator_activation` student ON raw.compliance_key = student.compliance_key AND raw.context.pipeline_context.submit_time BETWEEN student.licnese_grant_time AND student.licnese_expiration_time
...
JOIN `unity-other-learn-prd.reynafeng.educator_activation` student ON COALESCE(A.compliance_key,user.compliance_key) = student.compliance_key AND A.submit_time BETWEEN student.licnese_grant_time AND student.licnese_expiration_time
