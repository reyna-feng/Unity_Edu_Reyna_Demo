...
SELECT *,
       ...
       IF(NOT LEAD(grant_time) OVER(PARTITION BY license_record_id,license ORDER BY grant_time) IS NULL,LEAD(grant_time) OVER(PARTITION BY license_record_id,license ORDER BY grant_time),'2300-01-01') AS lead_grant_time,
       installation_limit - IF (NOT LAG(installation_limit) OVER(PARTITION BY license_record_id,license ORDER BY grant_time) IS NULL, LAG(installation_limit) OVER(PARTITION BY license_record_id,license ORDER BY grant_time), 0) AS grantCount
FROM(
...
SELECT body.license_record_id,
       ...
       IF(NOT DATE(LAG(body.expire_time) OVER(PARTITION BY body.license_record_id, body.license ORDER BY body.updated_time)) IS NULL, DATE(LAG(body.expire_time) OVER(PARTITION BY body.license_record_id, body.license ORDER BY body.updated_time)), DATE(body.expire_time)) AS lag_expire
